...
function main()
    ...
    for fname in ARGS
        d = h5open(fname)
        newname = fname[1:end-3]*"_strings.h5"
        ...
        pp_corrs = zeros(ComplexF64, 5, N, N)
        pm_corrs = zeros(ComplexF64, 5, N, N)
        for i0 = 1:N
            println("i=$i0")
            ...
            for (i,s) in enumerate(states)
                ...
                pp_corrs[i, i0, (i0+1):N] = [inner(s', op, s) for op in ops["PP"]]
                pm_corrs[i, i0, (i0+1):N] = [inner(s', op, s) for op in ops["PM"]]
                #println("s+s+")
                #println(round.(real.(pp_corrs[i, i0, i0:N]), digits=3))
                #println("s+s-")
                #println(round.(real.(pm_corrs[i, i0, i0:N]), digits=3))
            end
            println("ground state s+s+")
            println(round.(real(pp_corrs[1, i0, i0:N]), digits=3))
            println("ground state s+s-")
            println(round.(real(pm_corrs[1, i0, i0:N]), digits=3))
            println("Done measuring from $i0")
            println("|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||")
            println()
        end
